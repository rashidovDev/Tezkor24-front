...
const CourierAddUpdate = () => {
  ...
  async function createUser(data) {
    ...
    if (id) {
      ...
      response = await POST("/auth/registration-courier", data)
    }
    if (response) {
  ...
  return (
    <>
      ...
      <div className='h-[630px] px-16 relative mt-20'>
            ...
            <div>
              <form autoComplete='off' method="POST" onSubmit={handleSubmit(createUser)} action="">
                ...
                <div className="my-[40px] w-[33%] float-left">
                  ...
                  <input autoComplete="false" {...register("name", { required: true })} id='email' type="text" placeholder='Email'
                    className='border ml-1 border-sky-500 p-2 w-[300px] outline-[#5C3EBA]' />
                  {errors.email?.type === 'required' && <div className='text-danger mt-2'>Обязательное поле</div>}
                </div>
               ...
               <div className="my-[40px] w-[33%] float-left">
                  ...
                  <input autoComplete="false" {...register("phoneNumber", { required: true })} id='password' type="text" placeholder='Name'
                    className='border ml-1 border-sky-500 p-2 w-[300px] outline-[#5C3EBA]' />
                </div>
...
export default CourierAddUpdate
